...
import { CannotDeleteUserError, MissingParameterError } from "../../../../../src/domain/errors"
...
describe("delete-user", () => {
  ...
  it("should throw an error if repository retuns undefined", async () => {
    const { sut, deleteUserRepositorySpy } = makeSut()
    deleteUserRepositorySpy.output = undefined

    const promise = sut.execute("wrong_user_id")

    await expect(promise).rejects.toThrowError(new CannotDeleteUserError())
  })
})
